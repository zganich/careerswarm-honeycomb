...
- **Docs:** [docs/TAILOR_AND_INDUSTRY.md](docs/TAILOR_AND_INDUSTRY.md) — all nine personas, sectoral summary, new subsection **Page length by career level and industry** (tables for level and industry). [.cursor/rules/agent-prompts.mdc](.cursor/rules/agent-prompts.mdc) — Tailor persona list updated. [docs/CAREERSWARM_SUMMARY_FOR_MARKETING.md](docs/CAREERSWARM_SUMMARY_FOR_MARKETING.md) — industry-tailoring line under Who It's For.
- **Web FAQ** ([client/src/pages/FAQ.tsx](client/src/pages/FAQ.tsx)): Updated "How is this different?", "What is the Google XYZ format?", "Is this ATS-friendly?" to mention industry tailoring and role-specific formats. **New FAQs:** "What industries do you support?" (nine sectors, auto-detect); "How long should my resume be?" (entry 1p, mid 1–2p, senior 2–3p, federal 2p max, academic 2–4+, legal/education 1–2p).
- **Templates:** [shared/resumeTemplates.ts](shared/resumeTemplates.ts) — classic template `bestFor` extended with "Academic", "Education & Nonprofit".
- **Commit:** 9b593b7 pushed to main.
