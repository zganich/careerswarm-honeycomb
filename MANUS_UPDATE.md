...
**Copy everything below the line into a new Manus task.** This is the single source; use it from here or from `.cursor/commands/copy-paste-prompt-for-manus.md`.
...
- **Resume Roast:** Public API `public.roast({ resumeText })` in `server/routers.ts` (min 50 chars; no auth). Page at `/roast` (`client/src/pages/ResumeRoast.tsx`). Returns score, verdict, brutalTruth, 3 mistakes, characterCount, wordCount. LLM response parsing is robust: markdown code blocks stripped before JSON.parse; try/catch fallback returns safe default on parse error.
- **public.estimateQualification:** Stub procedure (currentRole, targetRole) returns score, gaps, reasoning for tests. Can be replaced with LLM later.
