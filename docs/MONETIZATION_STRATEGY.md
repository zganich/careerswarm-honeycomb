...
| OAuth (optional)  | Available when configured | Free             |
...
**Key insight:** Each side makes the other more valuable. Monetize both, but don't gate the connection — let free users apply to jobs, let recruiters see CareerSwarm-quality applicants. Gate the _volume_ and _advanced features_, not the core loop.
...
_This strategy is designed for a shoestring launch. Iterate based on real data. The goal is first revenue, not perfect revenue._
