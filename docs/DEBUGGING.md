...
| **Package generation fails / PDF missing** (1-Click Apply or Generate package) | Chromium not available in container (PDF uses `md-to-pdf` → Puppeteer/Chromium)                  | Railway’s default Node image has no Chromium. Install Chromium in Dockerfile (e.g. `apt-get` deps or `puppeteer` with `executablePath`) or use a PDF path that doesn’t require a browser. See [CRITICAL_SETUP_CHECKLIST.md](./CRITICAL_SETUP_CHECKLIST.md) § 4 (Storage) for Assembler; if PDF fails in prod, add Chromium to the image.                                            |
...
- **Package generation (PDF):** `server/services/pdfGenerator.ts` uses `md-to-pdf` (Puppeteer/Chromium). If PDF generation fails in production, the container may need Chromium installed (see Quick reference above).
