...
| **Railway healthcheck failure** (service unavailable)         | Probe used `/` which may not respond before timeout; or server not listening during `db:migrate` | Fixed: dedicated `GET /health` in `server/_core/index.ts` (returns 200 immediately); `railway.json` → `healthcheckPath`: `"/health"`. Redeploy. If migrate is slow, health checks retry 5m; if still failing, check deploy logs for migrate or startup errors.                                                                                                                      |
