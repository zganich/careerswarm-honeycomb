...
function getGhRepo() {
  const { stdout } = spawnSync('git', ['remote', 'get-url', 'origin'], { cwd: ROOT, encoding: 'utf8' });
  const m = (stdout || '').match(/github\.com[:/]([\w-]+\/[\w.-]+?)(?:\.git)?$/);
...
async function railwayLogs(lines = 15) {
  return new Promise((resolve) => {
    const proc = spawn('railway', ['logs'], { cwd: ROOT, stdio: ['ignore', 'pipe', 'pipe'] });
    let out = '';
    proc.stdout?.on('data', (d) => { out += d.toString(); });
    ...
    const t = setTimeout(() => {
      ...
      const hasError = ln.some((l) => /error|Error|ERR|500|failed|Fatal/i.test(l));
...
async function cloudflareStatus() {
  ...
  if (!token || !zoneId) return { ok: true, skipped: true, msg: 'CLOUDFLARE_API_TOKEN and CLOUDFLARE_ZONE_ID not set' };
...
async function appHealth() {
  try {
    const res = await fetch('https://careerswarm.com/api/health', { signal: AbortSignal.timeout(10000) });
...
async function runMonitor(notifyOnFail = false) {
  ...
  if (rw.ok) {
    ...
    if (notifyOnFail) notify('CareerSwarm', 'Railway status check failed', false);
  }
  console.log('');
  ...
  if (health.ok) {
    ...
    if (notifyOnFail) notify('CareerSwarm', `App health check failed: ${health.status || health.error}`, false);
  }
  console.log('');
  ...
  if (!logs.ok && logs.lines.length) {
    ...
    if (notifyOnFail) notify('CareerSwarm', 'Possible errors in Railway logs', false);
  } else {
