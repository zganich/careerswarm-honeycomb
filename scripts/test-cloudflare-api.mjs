...
async function main() {
  ...
  if (!zoneId) {
    console.log('   CLOUDFLARE_ZONE_ID not set — fetching zone ID for careerswarm.com...');
    const listRes = await fetch('https://api.cloudflare.com/client/v4/zones?name=careerswarm.com', {
    ...
    try {
      ...
      console.error('❌ Invalid JSON response when listing zones:', listRes.status, listRes.statusText);
      process.exit(1);
    }
    if (!listRes.ok) {
      console.error('❌ Failed to list zones:', listRes.status, listData.errors?.[0]?.message || JSON.stringify(listData.errors));
      process.exit(1);
    }
    if (!listData.success || !listData.result?.length) {
      console.error('❌ Failed to list zones:', listData.errors?.[0]?.message || 'No zones found');
      process.exit(1);
    }
  ...
  if (!res.ok) {
    console.error('❌ Zone fetch failed:', res.status, data.errors?.[0]?.message || JSON.stringify(data.errors));
    process.exit(1);
  }
