 ...
 */
test.describe
  .skip("Onboarding Flow - Complete Journey (use production-e2e.spec.ts for onboarding E2E)", () => {
  ...
  test("Step 1: Welcome page should display correctly", async ({ page }) => {
    ...
    await expect(
      page
        .getByText(/step 1 of 5/i)
        .or(page.getByRole("heading", { name: /welcome to careerswarm/i }))
    ...
    const heading = page.getByRole("heading", {
      name: /welcome|build your profile|get started/i,
    });
    await expect(heading).toBeVisible();
    const ctaButton = page
      .getByRole("button", { name: /build|start|continue|next/i })
      .or(page.getByRole("link", { name: /build|start|continue|next/i }));
  ...
  test("Step 3: Extraction page should show processing", async ({ page }) => {
    ...
    const extractionHeading = page.getByRole("heading", {
      name: /extract|processing|analyzing/i,
    });
    ...
    if (isVisible) {
      ...
      console.log(
        "Extraction still processing after 30s - this is expected with AI agents"
      );
    }
  });

  test("Step 4: Review page should display extracted data", async ({
    page,
  }) => {
    ...
    const continueButton = page.getByRole("button", {
      name: /continue|next|finish/i,
    });
    await expect(continueButton).toBeVisible();
  });

  test("Step 5: Preferences page should allow setting preferences", async ({
    page,
  }) => {
    ...
    const heading = page.getByRole("heading", {
      name: /preference|settings|complete/i,
    });
    await expect(heading).toBeVisible();
    ...
    const completeButton = page.getByRole("button", {
      name: /complete|finish|done|save/i,
    });
  ...
  test("Complete flow: Navigate through all 5 steps", async ({ page }) => {
    ...
    const uploadContinueButton = page.getByRole("button", {
      name: /continue/i,
    });
    ...
    console.log(
      "Waiting for extraction to complete (may take 30-60s due to LLM processing)..."
    );

    // Look for completion button or auto-navigation to review
    const extractionCompleteButton = page.getByRole("button", {
      name: /continue|review|next/i,
    });

    // Wait up to 60 seconds for extraction to complete
    const extractionComplete = await extractionCompleteButton
    ...
    if (extractionComplete) {
      ...
      const reviewContinueButton = page.getByRole("button", {
      ...
      console.log(
        "⚠️ Extraction did not complete within 60s - this may indicate LLM latency or processing issues"
      );
  ...
  test("Navigation: Back button should work on each step", async ({ page }) => {
    ...
    if (await backButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      console.log(
        "Back button not found - may use browser back or different navigation"
      );
    }
  });

  test("Validation: Cannot continue without uploading resume", async ({
    page,
  }) => {
    // Navigate to upload page
    await page.goto("/onboarding/upload");
  ...
  test("File upload: Can remove uploaded files", async ({ page }) => {
    ...
    const removeButton = page
      .locator("button")
      .filter({ hasText: /×|x|remove|delete/i })
      .first();
    ...
    if (await removeButton.isVisible({ timeout: 2000 }).catch(() => false)) {
      ...
      const fileStillVisible = await page
        .getByText(/test-resume\.pdf/i)
...
test.describe
  .skip("Onboarding Flow - Edge Cases (use production-e2e.spec.ts for onboarding E2E)", () => {
  test.beforeEach(async ({ page }) => {
    await bypassLogin(page);
  });
  ...
  test("Progress indicator: Shows correct percentage for each step", async ({
    page,
  }) => {
  ...
  test("Responsive design: Onboarding works on mobile viewport", async ({
    page,
  }) => {
    // Set mobile viewport
    ...
    const ctaButton = page
      .getByRole("button", { name: /build|start|continue/i })
      .or(page.getByRole("link", { name: /build|start|continue/i }))
      .first();
    await expect(ctaButton).toBeVisible();
