...
test.describe("Production Smoke Tests - Public Pages", () => {
  ...
  test("Homepage loads successfully", async ({ page }) => {
    ...
    const heroSection = page
      .locator('h1, [class*="hero"], [class*="Hero"]')
      .first();
  ...
  test("Dev Login page loads", async ({ page }) => {
    ...
    const heading = page.getByRole("heading", {
      name: /dev login|sign in|login/i,
    });
  ...
  test("Resume Roast page loads", async ({ page }) => {
    ...
    const heading = page
      .getByRole("heading", { name: /roast|resume/i })
      .first();
    ...
    const uploadArea = page
      .locator(
        'input[type="file"], textarea, [class*="upload"], [class*="drop"]'
      )
      .first();
...
test.describe("Production Smoke Tests - Public Pages (continued)", () => {
  ...
  test("Pricing page loads", async ({ page }) => {
    ...
    const heading = page
      .getByRole("heading", { name: /pricing|plans|subscribe/i })
      .first();
    await expect(heading).toBeVisible({ timeout: 10000 });
  ...
  test("FAQ page loads", async ({ page }) => {
    ...
    const heading = page
      .getByRole("heading", { name: /faq|frequently asked|questions|answers/i })
      .first();
    await expect(heading).toBeVisible({ timeout: 10000 });
    ...
    const faqButtons = page
      .locator("button")
      .filter({ has: page.locator('svg, [class*="chevron"]') });
  ...
  test("Terms of Service page loads", async ({ page }) => {
    ...
    const heading = page
      .getByRole("heading", { name: /terms|service|conditions/i })
      .first();
    await expect(heading).toBeVisible({ timeout: 10000 });
  ...
  test("For Recruiters page loads", async ({ page }) => {
    ...
    const hasRecruiterContent =
      text?.toLowerCase().includes("recruiter") ||
  ...
  test("No critical network errors on homepage", async ({ page }) => {
    ...
    const warningErrors = networkErrors.filter(
      err =>
    ...
    if (warningErrors.length > 0) {
      ...
      warningErrors.forEach(err => console.log(`  ${err.status} - ${err.url}`));
    }
...
test.describe("Production Smoke Tests - Mobile Responsive", () => {
  ...
  test("Homepage is mobile responsive", async ({ page }) => {
    ...
    const mobileNav = page
      .locator('[class*="mobile"], [class*="hamburger"], [class*="menu"]')
      .first();
    ...
    console.log(
      `✅ Homepage mobile responsive (mobile nav: ${isNavVisible ? "visible" : "hidden"})`
    );
  });
