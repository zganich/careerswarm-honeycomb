...
test.describe("Achievement Creation (STAR Wizard)", () => {
  ...
  test.skip('should display "Add Achievement" button on dashboard', async ({
    page,
  }) => {
    // Look for the add achievement button (there are multiple, so use .first())
    const addButton = page
      .getByRole("button", { name: /add achievement|new achievement/i })
      .first();
  ...
  test("should open STAR wizard when clicking add achievement", async ({
    page,
  }) => {
    // Find and click add achievement button
    const addButton = page
      .getByRole("button", {
        name: /add achievement|new achievement|create achievement/i,
      })
      .first();
    ...
    if (await addButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const hasSTARHeading = await page
        .getByText(/STAR|Situation|Task|Action|Result/i)
        .isVisible({ timeout: 3000 })
        .catch(() => false);
      const hasWizardDialog = await page
        .locator('[role="dialog"], .modal, .wizard')
        .isVisible({ timeout: 3000 })
        .catch(() => false);
  ...
  test("should show all STAR fields in the wizard", async ({ page }) => {
    ...
    if (await addButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const hasSituation = await situationField
        .isVisible({ timeout: 2000 })
        .catch(() => false);
      const hasTask = await taskField
        .isVisible({ timeout: 2000 })
        .catch(() => false);
      const hasAction = await actionField
        .isVisible({ timeout: 2000 })
        .catch(() => false);
      const hasResult = await resultField
        .isVisible({ timeout: 2000 })
        .catch(() => false);
  ...
  test("should validate required fields in STAR wizard", async ({ page }) => {
    ...
    if (await addButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const submitButton = page
        .getByRole("button", { name: /save|create|submit|next/i })
        .first();
      ...
      if (await submitButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const hasError = await page
          .getByText(/required|fill|enter|provide/i)
          .isVisible({ timeout: 2000 })
          .catch(() => false);
  ...
  test("should create achievement with valid STAR data", async ({ page }) => {
    ...
    if (await addButton.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (
        await situationField.isVisible({ timeout: 2000 }).catch(() => false)
      ) {
        await situationField.fill(
          "Our team needed to improve customer satisfaction scores"
        );
        await taskField.fill("Redesign the customer onboarding process");
        await actionField.fill(
          "Led a cross-functional team to map user journeys and implement improvements"
        );
        await resultField.fill(
          "Increased customer satisfaction by 35% and reduced churn by 20%"
        );
  ...
  test("should display created achievement in the list", async ({ page }) => {
    ...
    const achievementList = page.locator(
      '[data-testid="achievement-list"], .achievement-card, .achievement-item'
    );

    const hasAchievements = await achievementList
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
  ...
  test("should allow editing an existing achievement", async ({ page }) => {
    ...
    const achievementCard = page
      .locator(
        '[data-testid="achievement-card"], .achievement-card, .achievement-item'
      )
      .first();
  ...
  test("should allow deleting an achievement", async ({ page }) => {
    ...
    if (await achievementCard.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const deleteButton = achievementCard.getByRole("button", {
      ...
      if (await deleteButton.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const hasConfirmation = await page
          .getByText(/confirm|sure|delete/i)
          .isVisible({ timeout: 2000 })
          .catch(() => false);
  ...
  test("should show Impact Meter for achievements", async ({ page }) => {
    ...
    const impactMeter = page.locator(
      '[data-testid="impact-meter"], .impact-meter, [aria-label*="impact"]'
    );

    const hasImpactMeter = await impactMeter
      .first()
      .isVisible({ timeout: 3000 })
      .catch(() => false);
    ...
    if (hasImpactMeter) {
      ...
      console.log(
        "Impact Meter not found, might be in achievement detail view"
      );
    }
  });
  ...
  test("should show XYZ transformation for achievements", async ({ page }) => {
    ...
    const xyzFormat = page.getByText(
      /Accomplished \[X\]|by doing \[Y\]|as measured by \[Z\]/i
    );

    const hasXYZ = await xyzFormat
      .isVisible({ timeout: 3000 })
      .catch(() => false);
  ...
  test("should filter achievements by category", async ({ page }) => {
    ...
    if (
      ...
      const hasFilterOptions = await page
        .getByText(/leadership|technical|sales|all/i)
        .isVisible({ timeout: 2000 })
        .catch(() => false);
  ...
  test("should search achievements", async ({ page }) => {
    ...
    if (
      ...
      const hasResults = await page
        .locator('[data-testid="achievement-card"], .achievement-card')
        .isVisible({ timeout: 2000 })
        .catch(() => false);
