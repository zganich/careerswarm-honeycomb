    ...
    const animate = () => {
      ...
      particlesRef.current.forEach(particle => {
        ...
        if (particle.isLocked) {
          if (
            particle.springOffset !== undefined &&
            particle.springVelocity !== undefined
          ) {
            // Apply spring force
            const springForce = -particle.springOffset * 0.3; // Spring constant
            particle.springVelocity =
              (particle.springVelocity + springForce) * 0.85; // Damping
            particle.springOffset += particle.springVelocity;

            // Stop spring when settled
            if (
              Math.abs(particle.springOffset) < 0.1 &&
              Math.abs(particle.springVelocity) < 0.1
            ) {
      ...
      if (
        onGridCompletion &&
        completionPercentage !== lastCompletionRef.current
      ) {
