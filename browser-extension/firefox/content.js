...
(function () {
  ...
  function extractJobDescription() {
    ...
    if (url.includes("linkedin.com")) {
      title =
        document
          .querySelector(".top-card-layout__title, h1.topcard__title")
          ?.textContent?.trim() || "";
      company =
        document
          .querySelector(".topcard__org-name-link, .topcard__flavor")
          ?.textContent?.trim() || "";
      description =
        document
          .querySelector(".description__text, .show-more-less-html__markup")
          ?.textContent?.trim() || "";
    }
    // Indeed
      ...
      title =
        document
          .querySelector(".jobsearch-JobInfoHeader-title, h1")
          ?.textContent?.trim() || "";
      company =
        document
          .querySelector(
            ".icl-u-lg-mr--sm, .jobsearch-InlineCompanyRating-companyHeader a"
          )
          ?.textContent?.trim() || "";
      description =
        document
          .querySelector("#jobDescriptionText, .jobsearch-jobDescriptionText")
          ?.textContent?.trim() || "";
    }
    // Generic fallback
    else {
      ...
      company =
        document
          .querySelector('[class*="company"], [class*="Company"]')
          ?.textContent?.trim() || "";
      description =
        document
          .querySelector('main, [role="main"], article')
  ...
  async function analyzeJob() {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/intelligence.predictTrajectory`,
        {
          method: "POST",
          headers: {
      ...
      widget.querySelector(".cs-score-value").textContent =
