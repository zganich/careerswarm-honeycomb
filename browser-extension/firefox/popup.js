...
const API_BASE = 'https://3000-i9gyfqjd276sbiyfs99wv-0795b75c.us2.manus.computer';
...
async function checkLoginStatus() {
  try {
    ...
    if (data.result.data) {
      document.getElementById('status-text').textContent = `Logged in as ${data.result.data.name || data.result.data.email}`;
...
document.getElementById('analyze').addEventListener('click', async () => {
  ...
  if (!tab.url.includes('linkedin.com') && !tab.url.includes('indeed.com') && 
      !tab.url.includes('glassdoor.com') && !tab.url.includes('ziprecruiter.com')) {
    alert('Please navigate to a job posting on LinkedIn, Indeed, Glassdoor, or other job sites.');
    return;
  }
