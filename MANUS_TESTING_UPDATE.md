...
**Use `pnpm db:migrate` (not `db:push`).** The repo has a non-interactive script; see **MANUS_PROMPT.md** and **docs/RESPONSE_MANUS_TESTING.md**.

### Step 1: Initialize Database (REQUIRED)
```bash
pnpm db:migrate
# Verify: mysql -u [user] -p -e "SHOW TABLES;" [database_name]
# This codebase uses userProfiles, workExperiences, achievements, etc. (not a single masterProfiles table).
```

### Step 2: Add Error Handling
Done: parseResumes in server/routers.ts now throws if getDb() is null and has try/catch with user-facing TRPCError.

### Step 3: Re-test Onboarding Flow
```bash
pnpm dev
# Navigate to /login, sign in, complete onboarding Steps 1-5, verify data saved.
```
...
*Response and fixes: see docs/RESPONSE_MANUS_TESTING.md and MANUS_PROMPT.md*  
*Next Action: Run `pnpm db:migrate` then re-test onboarding.*
