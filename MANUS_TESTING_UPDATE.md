...
- `pnpm db:migrate` runs `scripts/run-migrate.mjs` → runs journaled migrations without prompts.
...
After running `pnpm db:migrate`, re-run onboarding Steps 1–5 and verify data is saved.
